...
class Slides extends MY_Controller
{
    ...
    public function save()
    {
        ...
        if ($image["success"]) :
            ...
            echo json_encode(["success" => false, "title" => "Başarısız!", "message" => "Slayt Kaydı Yapılırken Hata Oluştu. Slayt Mobil Görseli Seçtiğinizden Emin Olup Tekrar Deneyin."]);
            die();
        endif;
    ...
    public function update($id)
    {
        ...
        if (!empty($_FILES["mobile_url"]["name"])) :
            ...
            if ($image["success"]) :
                $data["mobile_url"] = $image["file_name"];
                if (!empty($slide->img_url)) :
                    if (!is_dir(FCPATH . "uploads/{$this->viewFolder}/{$slide->mobile_url}") && file_exists(FCPATH . "uploads/{$this->viewFolder}/{$slide->mobile_url}")) :
                        unlink(FCPATH . "uploads/{$this->viewFolder}/{$slide->mobile_url}");
                    endif;
                endif;
            else :
                echo json_encode(["success" => false, "title" => "Başarısız!", "message" => "Slayt Güncelleştirilirken Hata Oluştu. Slayt Görseli Seçtiğinizden Emin Olup Tekrar Deneyin."]);
