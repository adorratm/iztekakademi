    ...
    public function save()
    {
        ...
        if (checkEmpty($data)["error"] && checkEmpty($data)["key"] != "content" && checkEmpty($data)["key"] != "gallery_id") :
            ...
            if (!empty($_FILES)) :
                ...
                if (!empty($_FILES["pdf_url"]["name"])) :
                    $image = upload_picture("pdf_url", "uploads/$this->viewFolder", [], "*");
                    if ($image["success"]) :
                        $data["img_url"] = $image["file_name"];
                    else :
                        echo json_encode(["success" => false, "title" => "Başarısız!", "message" => "Sayfa Kaydı Yapılırken Hata Oluştu. Sayfa PDF Dosyası Seçtiğinizden Emin Olup Tekrar Deneyin."]);
                        die();
                    endif;
                endif;
    ...
    public function update($id)
    {
        ...
        if (checkEmpty($data)["error"] && checkEmpty($data)["key"] != "content" && checkEmpty($data)["key"] != "gallery_id") :
            ...
            if (!empty($_FILES["pdf_url"]["name"])) :
                $image = upload_picture("pdf_url", "uploads/$this->viewFolder", [], "*");
                if ($image["success"]) :
                    $data["pdf_url"] = $image["file_name"];
                    if (!empty($page->pdf_url)) :
                        if (!is_dir(FCPATH . "uploads/{$this->viewFolder}/{$page->pdf_url}") && file_exists(FCPATH . "uploads/{$this->viewFolder}/{$page->pdf_url}")) :
                            unlink(FCPATH . "uploads/{$this->viewFolder}/{$page->pdf_url}");
                        endif;
                    endif;
                else :
                    echo json_encode(["success" => false, "title" => "Başarısız!", "message" => "Sayfa Güncelleştirilirken Hata Oluştu. Sayfa PDF Dosyası Seçtiğinizden Emin Olup Tekrar Deneyin."]);
                    die();
                endif;
            endif;
            if (!empty($_FILES["banner_url"]["name"])) :
